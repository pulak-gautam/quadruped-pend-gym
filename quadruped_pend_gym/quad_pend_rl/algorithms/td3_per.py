...
class TD3():
    ...
    def run(self):
        ...
        for global_step in range(int(self.args['total_timesteps'])):
            ...
            if self.args['per']:
                self.rb.add((obs, actions, rewards, real_next_obs, terminations), rewards) #initial weight is rewards
            else:
            ...
            if global_step > int(self.args['learning_starts']):
                ...
                if self.args['per']:
                    self.qf1_loss = mse_with_is(self.qf1_a_values, next_q_value, is_weights)
                    self.qf2_loss = mse_with_is(self.qf2_a_values, next_q_value, is_weights)
                else:
                    self.qf1_loss = F.mse_loss(self.qf1_a_values, next_q_value)
                    self.qf2_loss = F.mse_loss(self.qf2_a_values, next_q_value)
                
                td_error = np.abs((self.qf1_a_values - next_q_value).detach().cpu().numpy())
                ...
                if self.args['per']:
                    self.rb.batch_update(idxs, td_error)
